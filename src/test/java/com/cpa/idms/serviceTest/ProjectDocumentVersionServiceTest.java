package com.cpa.idms.serviceTest;

import static org.junit.Assert.*;

import java.text.SimpleDateFormat;
import java.util.Date;

import org.junit.After;
import org.junit.Before;
import org.junit.Test;
import org.testng.Assert;

import com.cpa.idms.service.admin.dto.DepartmentDTO;
import com.cpa.idms.service.admin.dto.ProjectDocumentsDTO;

import io.restassured.http.Method;
import io.restassured.response.Response;
import io.restassured.specification.RequestSpecification;
import net.minidev.json.JSONObject;

public class ProjectDocumentVersionServiceTest {

	private static String tockenID = "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJhZG1pbiIsIlJvbGUiOiJbXSIsImV4cCI6MTYyMDYyNzE0Mn0.lO-NOLx7lAZp4KBXoWiPh8-IL-zfoqhLXkj7r1ZOCB3fPXuYR8ev0mw4IpwoZC4XYnkW_LaGDyeqU1NschVvSw";
	private static String baseUrl = "http://localhost:8080/api/rest";
	private static SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS");

	@Test
	public void testInsertProjectDocumentsVersion() {
		ProjectDocumentsDTO projectDocumentsDTO = new ProjectDocumentsDTO();
		io.restassured.RestAssured.baseURI = baseUrl;

		RequestSpecification httpRequestt = io.restassured.RestAssured.given();
		JSONObject requestParam = new JSONObject();

//		requestParam.put("projectDocumentsVersionId", "5");
		requestParam.put("activeFlag", "true");
		requestParam.put("docStatus", "DOne");
		requestParam.put("docVisibility", "Department");
		requestParam.put("estimateDay","10");
		requestParam.put("expiryDate","13");
		requestParam.put("pdfConvertDate","21");
		requestParam.put("pdfConvertStatus","Done");
		requestParam.put("pdfObjectId","1");
		requestParam.put("pdfPath","CSE");
		requestParam.put("rawFilePath","SSS");
		requestParam.put("versionNo","1");
		requestParam.put("webConvDate","21");
		requestParam.put("webCovStatus","Done");
		requestParam.put("webObjectId","1");
		requestParam.put("webPath","XLS");
		requestParam.put("projectDocumentsId","1");
		requestParam.put("createdBy", "Pawan");
		requestParam.put("createdDate", dateFormat.format(new Date()));
		requestParam.put("modifiedBy", "Kumar");
		requestParam.put("modifiedDate", dateFormat.format(new Date()));
		requestParam.put("filed1","Testing");
		requestParam.put("filed2","Testing");
		requestParam.put("filed3","Testing");
		requestParam.put("filed4","Testing");
		
		httpRequestt.header("Content-Type", "application/json");
		httpRequestt.body(requestParam.toJSONString());

		Response responsee = httpRequestt.request(Method.POST, "/createProjectDocumentsVersion");

		int StatusCode=responsee.getStatusCode();
		Assert.assertEquals(StatusCode,200);  
		System.out.println("Status code is " +StatusCode);
	}

	@Test
	public void testRetrieveProjectDocumentsVersionById() {
		io.restassured.RestAssured.baseURI = baseUrl;


		RequestSpecification httpRequest = io.restassured.RestAssured.given();

		//JSONObject requestParam = new JSONObject();

		//requestParam.put("id","1");

		Response response = httpRequest.request(Method.GET, "/getDocumentsVersionWorkflowById?id=1");

		String responseBody = response.getBody().asString();
		System.out.println("Response Body is =>  " + responseBody);
	}

	@Test
	public void testRetrieveAllProjectDocumentsVersion() {
      io.restassured.RestAssured.baseURI = baseUrl;
		
		RequestSpecification httpRequest = io.restassured.RestAssured.given();

		Response response = httpRequest.request(Method.GET, "/getAllProjectDocumentsVersion");

		String responseBody = response.getBody().asString();
		System.out.println("Response Body is =>  " + responseBody);
	}

	@Test
	public void testUpdateProjectDocumentsVersion() {
		DepartmentDTO deptDto = new DepartmentDTO();
		io.restassured.RestAssured.baseURI = baseUrl;

		RequestSpecification httpRequestt = io.restassured.RestAssured.given();
		JSONObject requestParam = new JSONObject();
//		requestParam.put("projectDocumentsVersionId", "5");
		requestParam.put("activeFlag", "true");
		requestParam.put("docStatus", "DOne");
		requestParam.put("docVisibility", "SSSSSS");
		requestParam.put("estimateDay","10");
		requestParam.put("expiryDate","5");
		requestParam.put("pdfConvertDate","11");
		requestParam.put("pdfConvertStatus","Done");
		requestParam.put("pdfObjectId","1");
		requestParam.put("pdfPath","CSE");
		requestParam.put("rawFilePath","SSS");
		requestParam.put("versionNo","1");
		requestParam.put("webConvDate","15");
		requestParam.put("webCovStatus","Done");
		requestParam.put("webObjectId","1");
		requestParam.put("webPath","XLS");
		requestParam.put("projectDocumentsId","1");
		requestParam.put("createdBy", "Pawan");
		requestParam.put("createdDate", dateFormat.format(new Date()));
		requestParam.put("modifiedBy", "Singh");
		requestParam.put("modifiedDate", dateFormat.format(new Date()));
		requestParam.put("filed1","Testing");
		requestParam.put("filed2","Testing");
		requestParam.put("filed3","Testing");
		requestParam.put("filed4","Testing");
		
		httpRequestt.header("Content-Type", "application/json");
		httpRequestt.body(requestParam.toJSONString());

		Response responsee = httpRequestt.request(Method.PUT, "/updateProjectDocumentsVersion");

		int StatusCode=responsee.getStatusCode();
		Assert.assertEquals(StatusCode,200);  
		System.out.println("Status code is " +StatusCode);
	}

	@Test
	public void testDeleteById() {
		io.restassured.RestAssured.baseURI = baseUrl;

		RequestSpecification httpRequestt = io.restassured.RestAssured.given();
		JSONObject requestParam = new JSONObject();
		httpRequestt.header("Content-Type", "application/json");
		//JSONObject requestParam = new JSONObject();
		//requestParam.put("id","1");
		httpRequestt.body(requestParam.toJSONString());

		Response responsee = httpRequestt.request(Method.DELETE, "/removeProjectDocumentsVersionById?id=1");
		int StatusCode=responsee.getStatusCode();
		Assert.assertEquals(StatusCode,200);  
		System.out.println("Status code is " +StatusCode);
	}

}
